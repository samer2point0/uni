...
class AuctionClient(object):
    ...
    def first_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        i=0
        st=(numberbidders/len(remaining))
        disc={'total':0}
        for item in remaining:
            have=standings[mybidderid][item]
            disc['total']=disc['total']+pow(discount,i)+st*have
            if item in disc:
                disc[item]=disc[item]+pow(discount,i)+st*have
            else:
                disc[item]=pow(discount,i)+st*have
            i=i+1

        r=random.random()
        rat=disc[artist]/disc['total']
        nrat=rat
        f=0.1
        print(rat)
        i=0
    ...
    def second_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        discount=0.98
        remaining=0
        for item in artists.keys():
            remaining=remaining+artists[item]
        i=0
        st=(numberbidders/remaining)
        disc={'total':0}
        for item in artists.keys():
            have=standings[mybidderid][item]
            sold=winnerarray.count(item)
            disc['total']=disc['total']+(artists[item]-sold)*st*(have+0.2)
            disc[item]=(artists[item]-sold)*st*(have+0.2)
    ...
    def third_bidding_strategy(self, numberbidders, wincondition, artists, values, rd, itemsinauction, winnerarray, winneramount, mybidderid, players, standings, winnerpays):
        ...
        i=0
        for item in remaining:
            totval=totval+values[item]*pow(discount,i)
            i=i+1
        r=random.random()
        price=v*budget/totval
        return int(price)
